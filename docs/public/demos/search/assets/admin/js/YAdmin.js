...
// PostBase 已简化，移除 access_token（SAAS 使用 Session 认证）
var PostBase = {
    plat: "admin",
    version: "10000"
};

// 为 qfadmin 页面统一注入头像下拉邀请信息能力
    ...
    Vue.mixin({
        data: function () {
            return {
                ...
                headerInviteInfoLoading: false,
                headerInviteInfoLoaded: false,
                instanceOwnerUnbinding: false
            };
        },
        methods: {
            onHeaderUserDropdownVisible: function (visible) {
                if (!visible || this.headerInviteInfoLoaded || this.headerInviteInfoLoading) {
                    return;
                }
                this.fetchHeaderInviteInfo();
            },
            fetchHeaderInviteInfo: function () {
                var that = this;
                if (that.headerInviteInfoLoading) {
                    return;
                }
                that.headerInviteInfoLoading = true;
                axios.post('/qfadmin/saas_auth/getInviteInfo', Object.assign({}, PostBase))
                    .then(function (res) {
                        if (res.data && res.data.code == CODE_SUCCESS && res.data.data) {
                            var data = res.data.data;
                            that.headerInviteInfo = {
                                invite_code: data.invite_code || '',
                                invited_count: data.invited_count || 0,
                                invited_users: Array.isArray(data.invited_users) ? data.invited_users : []
                            };
                            that.headerInviteInfoLoaded = true;
                        }
                        that.headerInviteInfoLoading = false;
                    })
            ...
            unbindInstanceOwner: function () {
                var that = this;
                if (that.instanceOwnerUnbinding) {
                    return;
                }

                that.instanceOwnerUnbinding = true;
                that.$confirm('解绑后当前会话会立即退出；下一个登录账号将成为 owner，并接管本实例现有资源和配置。', '解除实例绑定', {
                    confirmButtonText: '解除绑定',
                    cancelButtonText: '取消',
                    type: 'warning',
                    confirmButtonClass: 'el-button--danger'
                }).then(function () {
                    return axios.post('/qfadmin/saas_auth/unbindOwner', Object.assign({}, PostBase));
                }).then(function (res) {
                    if (!res.data || res.data.code != CODE_SUCCESS) {
                        throw new Error((res.data && res.data.message) || '解除实例绑定失败');
                    }
                    that.$message.success(res.data.message || '已解除实例绑定');
                    var redirect = res.data.data && res.data.data.redirect;
                    window.location.replace(redirect || '/qfadmin/saas_auth/login');
                }).catch(function (error) {
                    if (error !== 'cancel' && error !== 'close') {
                        that.$message.error(error.message || '解除实例绑定失败');
                    }
                }).finally(function () {
                    that.instanceOwnerUnbinding = false;
                });
            }
        }
    });
...
function logout() {
    ...
    if (r==true){
        // SAAS 登出 - 使用新的 SAAS 认证端点
        axios.post('/qfadmin/saas_auth/logout', PostBase)
            .then(function (res) {
                if (res.data.code == 200) {
                    // 检查是否有redirect字段
                    if (res.data.data && res.data.data.redirect) {
                        window.location.href = res.data.data.redirect;
                    } else {
                        window.location.href = "/qfadmin/saas_auth/login";
                    }
                } else {
